...
public class ReportFoundActivity extends Activity implements OnDateSetListener,
		OnTimeSetListener {
	// fields
	...
	private File currentCachedImage;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// dummy
		// String[] cats =
		// getResources().getStringArray(R.array.foundcat_strings);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		Log.d("RFOUND", "Entering onActivityResult " + resultCode + ":"
				+ requestCode);
		if (resultCode == RESULT_OK) {
			Log.d("RFOUND", "RESULT_OK");
			switch (requestCode) {
			case GALLERY_RCODE:
				...
				break;
			case CAMERA_RCODE:
				Log.d("IMG_CAM", currentCachedImage.getAbsolutePath());
				if (uploadedImages.size() < MAX_PIC_NUMBER) {
					String location = currentCachedImage.getAbsolutePath();
					Bitmap image = ImageHandler.decodeFile(location, 500, 500);
					uploadedImages.add(image);
					imagesLocation.add(location);
					galleryAdapter.notifyDataSetChanged();
	...
	private void startNativeCamera() {
		// prepare file
		Log.d("FCAM", "Starting Camera");
		try {
			currentCachedImage = prepareCacheImage();
			Uri outputUri = Uri.fromFile(currentCachedImage);
	...
	private void getGPSLocation() {
		...
		if (gps.canGetLocation()) {
			...
			String dummy = (new LocationAddress(this, latitude, longitude))
					.getAddress();
			if (dummy != null) {
	...
	private File prepareCacheImage() throws IOException {
		Calendar cal = Calendar.getInstance(TimeZone.getDefault());
		String timeStamp = new SimpleDateFormat("yyyyMMdd_HHmmss").format(cal
				.getTime());
		File temp = File
				.createTempFile(
						"alphacloud_" + timeStamp,
						".jpg",
						Environment
	...
	class ReportFound extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... args) {
			...
			// Log.d("report_found_data",cat + " " + date + " " + time + " " +
			// user + " " + email + " " + phone + " " + desc);
			...
			// getting JSON Object
			JSONObject json = jsonParser.makeHttpRequest(DbConn.REPORT_FOUND,
					"POST", params);
			...
			try {
				...
				if (success == 1) {
					...
					for (int i = 0; i < iNames.length; i++) {
						...
						iNames[i] = "f" + id + "_" + timeStamp + "_" + i + ".jpg";
					}
					// upload image
		...
		private void uploadImagesDB(int id, String filename) {
			...
			// getting JSON Object
			JSONObject json = jsonParser.makeHttpRequest(DbConn.SAVE_PIC_FOUND,
					"POST", params);
